 ...
 */
@RestController
@RequestMapping("/admin/acl/filter")
...
public class MscAclFilterController {
    ...
    public ResponseData<MscAclFilter> save(@RequestBody MscAclFilter mscAclFilter) {
        MscAclFilter exists = dao.queryForObject(MscAclFilter.class, "select * from msc_acl_filter where saas_id=? and user_type=? and user_id=? and state>?", new Object[]{mscAclFilter.getSaasId(), mscAclFilter.getUserType(), mscAclFilter.getUserId(), CommonState.DELETED.getValue()}).getData();
        if (exists != null) {
