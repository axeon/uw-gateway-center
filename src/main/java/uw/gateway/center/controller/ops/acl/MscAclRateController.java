...
public class MscAclRateController {
    ...
    @MscPermDeclare(user = UserType.OPS, auth = AuthType.USER, log = ActionLog.NONE)
    public ResponseData<MscAclRateResult> testRateLimit(@Parameter(description = "saasId") @RequestParam long saasId, @Parameter(description = "用户类型") @RequestParam int userType, @Parameter(description = "用户Id") @RequestParam long userId, @Parameter(description = "资源地址") @RequestParam String uri, @Parameter(description = "IP许可") @RequestParam String ipPermit, @Parameter(description = "请求数许可") @RequestParam int requestsPermit, @Parameter(description = "字节数许可") @RequestParam int bytesPermit) {
